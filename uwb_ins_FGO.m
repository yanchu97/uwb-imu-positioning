function [e_FGO,nav_e_fgo,e5] = uwb_ins_FGO(imu2, uwb, ref)
...
[uwb.X,uwb.Y] =fun_PDA(ref.x(1,1),ref.y(1,1),uwb.range_30,BS,0,0.1,4,0.5);
...
% 启动主循环：在每个时间步执行推理
for measurementIndex = firstuwbPosition:815
    ...
    if measurementIndex == firstuwbPosition
        ...
        % Bias evolution as given in the IMU metadata IMU 元数据中给出的偏差演化
...
nav_e_fgo.x(1)=ref.x(1,1);
nav_e_fgo.y(1)=ref.y(1,1);
nav_e_fgo.z(1)=ref.z(1,1);
nav_e_fgo.x(2:20)=result_Position(1,10);
nav_e_fgo.y(2:20)=result_Position(1,11);
nav_e_fgo.z(2:20)=result_Position(1,12);
for i=1:813
    nav_e_fgo.x(20*i+1:20*(i+1))=result_Position(i+1,10);
    nav_e_fgo.y(20*i+1:20*(i+1))=result_Position(i+1,11);
    nav_e_fgo.z(20*i+1:20*(i+1))=result_Position(i+1,12);
%     e5(20*i-19:20*i)=sqrt(((nav_e_fgo.x(20*i)-ref.x(20*i))^2+(nav_e_fgo.y(20*i)-ref.y(20*i))^2));
end
e_FGO=0;
for i=1:16280
    e_FGO=e_FGO+((nav_e_fgo.x(i)-ref.x(i))^2+(nav_e_fgo.y(i)-ref.y(i))^2);
    e5(i)=sqrt(((nav_e_fgo.x(i)-ref.x(i))^2+(nav_e_fgo.y(i)-ref.y(i))^2));
end
end