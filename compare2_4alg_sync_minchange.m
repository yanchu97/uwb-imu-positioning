...
rmset5=zeros(L,1);
    ...
    if strcmp( EKF , 'ON' )
        ...
        rmse11 = hypot(x_ref_eval - x_nav, y_ref_eval - y_nav);
rmset11 = rmset11 + rmse11;
    ...
    if strcmp( SHFAEKF , 'ON' )
        ...
        y_nav = interp1(t_nav, y_series, t_eval, 'linear', 'extrap');
        z_nav = interp1(t_nav, z_series, t_eval, 'linear', 'extrap');
        rmse4 = hypot(x_ref_eval - x_nav, y_ref_eval - y_nav);
if mean(rmse4) < 5
            rmset4 = rmset4 + rmse4;
        else
            T1 = T1 - 1;
    ...
    if strcmp( PRPF , 'ON' )
        ...
        y_nav = interp1(t_nav, y_series, t_eval, 'linear', 'extrap');
        z_nav = interp1(t_nav, z_series, t_eval, 'linear', 'extrap');
        rmse3 = hypot(x_ref_eval - x_nav, y_ref_eval - y_nav);
if mean(rmse3) < 5
            rmset3 = rmset3 + rmse3;
        else
            T2 = T2 - 1;
    ...
    if strcmp( FGO , 'ON' )
        ...
        tic
        [nav2_e5] = uwb_ins_FGO2(imu2, uwb, num_anchor, ref);
        ...
        rmse5 = hypot(x_ref_eval - x_fgo, y_ref_eval - y_fgo);
if mean(rmse5) < 5
            rmset5 = rmset5 + rmse5;
        else
            T_FGO = T_FGO - 1;
